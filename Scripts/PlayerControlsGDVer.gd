...
@export var cur_phys_item_held: HeldItemGDVer = null
var item_held_num = 3 #3 stands for "Nothing
var items = {'Nothing': null, 'Compass': PackedScene, 'Flashlight': preload("res://flashlight_standin.tscn"), 'Map': PackedScene}
...
@onready var player_animation: AnimationPlayer = get_node("Pivot/AnimationPlayer")
...
func _ready():
	...
	#connect(player_animation.animation_finished, finished_anim)
...
func handle_input(delta: float):
	...
	if not changing_held:
		if number_to_item.get(item_held_num) != cur_item_held:
			...
			if Input.is_action_just_pressed("left_activate_item") && cur_phys_item_held != null:
				cur_phys_item_held.activate()
...
func handle_changing(delta: float):
	...
	if changing_held:
		if cur_phys_item_held != null:
			cur_phys_item_held.unequip_item()
			cur_phys_item_held.queue_free()
			cur_phys_item_held = null
		var to_equip = items.get(item_to_change_to)

		if to_equip != null:
			var equip_this = to_equip.instantiate()
			get_node("Pivot/MyEyes/My arms").add_child(equip_this)
			equip_this.equip_item()
			cur_phys_item_held = equip_this
			cur_item_held = item_to_change_to
		else:
			cur_phys_item_held = null
			cur_item_held = 'Nothing'
		changing_held = false
		item_to_change_to = ''
	...
	# 	if cur_phys_item_held.item_is_held:
	# 		#unequip it
	# 		unequipped = cur_phys_item_held.unequip_item()
	# 	else:
...
func _physics_process(delta):
	control(delta)
	handle_input(delta)
	handle_changing(delta)

func has_enough_keys(num_of_keys: int) -> bool:
	if keys_carried >= num_of_keys:
		return true
	else:
		return false
func take_keys(num_of_keys: int):
	keys_carried -= num_of_keys

func return_keys(num_of_keys: int):
	keys_carried += num_of_keys
